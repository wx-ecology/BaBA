...
movement.df.all <- read.csv("xxx")  # read in your movement data. 
...
for (i in unique(movement.df.all$Location.ID)) {
  ...
  for (ii in (b+1):min(p, nrow(movement.df.i))) { # ii is the window size (duration of trajectories that straightness will be calculated)
    ...
    for (iii in seq(1, (nrow(movement.df.i)-ii), by = 2)) {  # can change "by" for different sampling rate to calculate strightness to save some time
      ...
      # Mark the starting time of the trajectory
      date.iii <- as.character(strftime(mov.seg.iii$date[1], "%Y-%m-%d %H:%M:%S")) 
      # calculate straightness of this traijectory
...
write.csv(animal.stn.df, paste0(getwd(), "xxxx"))
